...
import { CreateattributeValue, DeleteattributeValue, UpdateattributeValue } from '../../apis/attributevalue';
...
const AttributeValue = () => {
    ...
    const [attributeValueData, setattributeValueData] = useRecoilState(attributeValueDataState)
    ...
    const addcategoryAndValueInput = () => {
        setcategoryAndValueInputs([...categoryAndValueInputs,{
            category : "",
            attributeValue : ""
        }])
    ...
    const createattributeValue = async (values) => {
        try {
            ...
            const categoryAndValueArray = [];
             console.log(values,"vvvvvvvvvvvvvvvvvv")
             console.log(categoryAndValueInputs,"vcccccccccccccccvvvvvvvvvvvvvvvvv")
            // Iterate over each category and value input
            categoryAndValueInputs.forEach((input) => {
                // Extract category and attribute value from the input
                ...
                // console.log(categoryAndValueArray,'arrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr')
            });
            ...
            if (response.success) {
                ...
                console.log(attributeValueData,'responseeeeeeeeeeeeeeeee1')

                setattributeValueData([...attributeValueData, response.attributeValueDoc]);
                console.log(attributeValueData,'responseeeeeeeeeeeeeeeee2')
    ...
    const deleteItem = async (id) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setattributeValueData((prevData) => prevData.filter((attributeValue) => attributeValue._id !== id));
    ...
    const updateFormWithattributeValueData = (attributeValueId, updatedattributeValueData) => {
        const attributeValueDataexist = updatedattributeValueData.find((element) => element._id === attributeValueId);
        console.log(attributeValueDataexist,attributeValueDataexist?.valuesCombo?.category,'existssssssssssssssssssssss');
        setrefcat(() => (attributeValueDataexist?.valuesCombo[0]?.category))
        setrefatt(() => (attributeValueDataexist?.attributeRef))
        
     console.log(attributeValueDataexist?.valuesCombo[0].attributeValue,'vvvvvvvvvvvvvvvv')
     const valuesCombo = attributeValueDataexist?.valuesCombo; // Access the first element directly
     console.log(valuesCombo?.attributeValue, 'ffffffffffffffffffffffffffff');
    ...
    // Use updateFormWithattributeValueData in the useEffect
    useEffect(() => {
        updateFormWithattributeValueData(updateId, attributeValueData);
        setUpdated(false);
    }, [updated]);

    // ...

    // Call updateFormWithattributeValueData wherever needed
    const handleUpdate = (attributeValueId) => {
        try {
            setUpdated(true)
            updateFormWithattributeValueData(attributeValueId, attributeValueData);
    ...
    const handleUpdateSubmit = async (values) => {
        try {
            ...
            if (response.success) {
                ...
                const updatedattributeValues = attributeValueData.map((attributeValue) =>
                    attributeValue._id === updateId ? response.attributeValue : attributeValue
                );
    ...
    return (
        <>
            <div className="flex flex-col gap-2">
                ...
                <div key={index} className="flex gap-2">
                                            ...
                                            <input
                                                autoFocus
                                                onChange={(event) => {
                                                    // Update the attribute value for the corresponding input
                                                    formik.setFieldValue("attributeValue", event.target.value)
                                                    const updatedInputs = [...categoryAndValueInputs];
                                                    updatedInputs[index].attributeValue = event.target.value;
                                                    
                                                ;}}
                                                value={formik.values?.valuesCombo[index]?.attributeValue}
            ...
            <Button
                onClick={addcategoryAndValueInput}
            >
            ...
            <DataTableModel visible_columns={INITIAL_VISIBLE_COLUMNS} deleteItem={deleteItem} update={handleUpdate} columns={columns} statusOptions={statusOptions} users={attributeValueData} onOpen={onOpen} section={'attributeValue'} />
        </>
    )
}
